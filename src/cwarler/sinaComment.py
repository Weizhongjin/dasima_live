...
class WeiboCommentScrapy(Thread):
    ...
    def write_to_csv(self,wid,result,isHeader=False):
        with open('comment/' + wid + '.csv', 'a', encoding='utf-8-sig', newline='') as f:
            writer = csv.writer(f)
            if isHeader == True:
                writer.writerows([self.result_headers])
            writer.writerows(result)
        print('已成功将{}条评论写入{}中'.format(len(result),'comment/' + wid + '.csv'))

    def scrapy(self,wid):
        res = requests.get('https://weibo.cn/comment/{}'.format(wid),headers=self.headers,verify=False)
        ...
        for page in range(pageNum):
            ...
            res = requests.get('https://weibo.cn/comment/{}?page={}'.format(wid,page+1), headers=self.headers,verify=False)
            ...
            for i in range(len(comments)):
                try:
                    res = self.get_one_comment_struct(comments[i])
                    result.append(res)
                except:
                    print("something wrong with : {}".format(comments[i]))

            if page==0:
                self.write_to_csv(wid,result,isHeader=True)
            else:
                self.write_to_csv(wid,result,isHeader=False)

            sleep(randint(1,5))

if __name__ =="__main__":
    Scrapier = WeiboCommentScrapy(header=headers)
    Scrapier.scrapy(wid='Kqdbg22FF')